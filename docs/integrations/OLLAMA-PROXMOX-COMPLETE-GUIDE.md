...
- ✅ Работает с майнинговыми картами (P106, P104, CMP)
- Ubuntu 22.04 LTS VM
...
**Если нет (AMD):** Это нормально на данном этапе - IOMMU будет активирован после настройки GRUB (Этап 2). Главное чтобы `svm` был виден.

### 1.3 Специфика для ASRock материнских плат (важно!)

Если у вас **ASRock** (например B450M Pro4, X570 и т.д.) и вы **не видите опцию IOMMU в BIOS**:

1. **Проверьте версию BIOS:**
   - Войдите в BIOS и посмотрите версию (обычно на главном экране)
   - Для B450M Pro4: старые версии (P3.xx) могут не показывать IOMMU

2. **Обновите BIOS до последней версии:**
   - Скачайте с официального сайта ASRock: https://www.asrock.com/support/
   - Используйте **Instant Flash** в BIOS (Tools → Instant Flash)
   - Скопируйте файл .ROM на USB флешку FAT32
   - В новых версиях (P7.xx+) опция IOMMU появляется

3. **После обновления BIOS ищите:**
...
4. **Также включите:**
   - SVM Mode → Enabled (если еще не включено)

**⚠️ Важно:** Без обновления BIOS опция IOMMU может быть скрыта даже на поддерживаемом процессоре!
...
**⚠️ Важно для AMD:** Используйте именно `amd_iommu=on`, а не `amd_iommu=1` или `amd_iommu=force_enable`. В современных ядрах (6.8+) работает только `=on`. После перезагрузки может появиться предупреждение "AMD-Vi: Unknown option - 'on'" - игнорируйте его, AMD-Vi все равно инициализируется.
...
**Для AMD: дополнительная проверка IVRS**

Если AMD-Vi не инициализируется, проверьте что BIOS передает таблицу IVRS:
...
```bash
# 2. Проверка IOMMU групп (критично!)
...
```bash
# 3. Проверка AMD-Vi инициализации
dmesg | grep -i amd-vi
...
**Если IOMMU groups = 0:**
1. Проверьте `dmesg | grep -i ivrs` - если пусто, IOMMU не включен в BIOS
2. Проверьте `/proc/cmdline` - должно быть `iommu=pt`
3. Повторите Этап 1 (BIOS) и 2.1-2.4
...
**Пример вывода для P106-100 (майнинговая карта):**
```text
06:00.0 3D controller [0302]: NVIDIA Corporation GP106 [P106-100] [10de:1c07] (rev a1)
```

**Запишите:**
- PCI Bus адрес: `01:00.0` (GPU), `01:00.1` (Audio, если есть)
- Vendor:Device ID: `10de:1c82` (GPU), `10de:0fb9` (Audio, если есть)

**⚠️ Важно для майнинговых карт (P106, P104, CMP):**
- У них **НЕТ Audio контроллера** (01:00.1)
- У них **НЕТ видеовыходов** (HDMI/DisplayPort)
- Это **нормально** - они полностью работают для compute задач (Ollama, CUDA)
- В vfio.conf указывайте **только GPU ID** без Audio
...
**Для обычных GTX/RTX карт (с Audio):**
```bash
echo "options vfio-pci ids=10de:1c82,10de:0fb9" > /etc/modprobe.d/vfio.conf
```

**Для майнинговых карт (P106, P104, без Audio):**
```bash
echo "options vfio-pci ids=10de:1c07" > /etc/modprobe.d/vfio.conf
```

**Замените** на ваши ID из шага 3.1:
- Если у вас **два** устройства (GPU + Audio): укажите оба через запятую
- Если **одно** устройство (майнинговая карта): укажите только GPU ID
...
**Ожидаемый вывод для GTX/RTX (с Audio):**
```text
...
**Ожидаемый вывод для майнинговых карт (P106, P104):**
```text
06:00.0 3D controller [0302]: NVIDIA Corporation GP106 [P106-100] [10de:1c07] (rev a1)
...
**Критерий успеха:** `Kernel driver in use: vfio-pci` для GPU (и Audio, если есть).
...
## Этап 4.5: Проверка доступной памяти на хосте (критично!)

**Перед созданием VM проверьте RAM на Proxmox хосте:**
...
**Формула:** `RAM для VM = Total RAM хоста - 2GB (для Proxmox) - запас 1GB`

**Примеры:**
- **16GB хост:** VM может иметь до 13GB (рекомендуется 10-12GB)
- **8GB хост:** VM может иметь до 5GB (рекомендуется 4-6GB) ⚠️
- **32GB хост:** VM может иметь до 29GB (рекомендуется 16-24GB)

### ⚠️ Для данного руководства:

**Минимальная конфигурация в руководстве:**
- **RAM хоста:** 8GB минимум
- **RAM VM:** 8GB (для комфортной работы с Ollama)

**Если у вас меньше 10GB на хосте:**
- Уменьшите RAM VM до 4-6GB
- Используйте только небольшие модели (phi3:mini, llama3.2:3b)
- Рассмотрите возможность увеличения RAM хоста

**Наш пример (AMD Ryzen 5 2600 + 8GB):**
- **Текущая конфигурация:** 7.7GB RAM → VM с 6GB RAM
- **Рекомендуемое обновление:** 16GB RAM → VM с 10-12GB RAM
- **После обновления:** Сможем запускать модели до llama3.1:8b

### 📝 Примечание:
В данной установке использовался хост с 8GB RAM (ограничение железа). VM настроена на 6GB RAM с моделью phi3:mini (2.3GB). **Планируется обновление RAM хоста до 16GB** для возможности запуска более крупных моделей (llama3.1:8b, qwen2.5:7b).
...
⚠️ **Выберите в зависимости от RAM хоста (см. Этап 4.5):**

**Если RAM хоста >= 16GB:**
- **Memory (MiB):** `8192` (8GB, оптимально для Ollama)
- **Minimum memory (MiB):** `8192`

**Если RAM хоста 8-10GB:**
- **Memory (MiB):** `6144` (6GB, для небольших моделей)
- **Minimum memory (MiB):** `6144`

**Если RAM хоста >= 32GB:**
- **Memory (MiB):** `12288` (12GB, для крупных моделей 8B+)
- **Minimum memory (MiB):** `12288`

- **Ballooning Device:** ❌ Uncheck (отключите для стабильности)

**Примечание:** В нашем примере (8GB хост) используется 6GB для VM.
...
2. **Raw Device:** выберите вашу NVIDIA GPU (01:00.0 или 06:00.0)
3. Установите галочки:
   - ✅ **All Functions** (важно - захватит GPU и Audio вместе, если есть Audio)
...
⚠️ **Важно для майнинговых карт (P106, P104, CMP):**
- У них нет Audio контроллера, поэтому "All Functions" захватит только GPU
- **НЕ включайте x-vga=1** - это может вызвать проблемы с запуском VM
- Для обычных GTX/RTX карт x-vga=1 можно включить
...
**⚠️ Важно при ручном редактировании:**
1. Proxmox Web UI уже создал строку `cpu: host`
2. Вы добавите улучшенную версию `cpu: host,hidden=1,flags=+pcid`
3. **Удалите старую строку** `cpu: host` (без параметров)
4. Оставьте только новую строку с параметрами

Найдите строку `cpu: host` (без параметров) и **замените её** на:
...
Должна быть **только одна** строка с cpu!
...
**⚠️ Важно:** Если `systemctl edit` не создает файл автоматически, создадим вручную.

#### Вариант A: Через systemctl edit (рекомендуется):

```bash
sudo systemctl edit ollama.service
```

Откроется редактор. Добавьте **только эти строки** (между комментариями):
...
#### Вариант B: Если Вариант A не работает (файл не создается):
...
Должны увидеть 4 строки с настройками.
...
**Должно быть:** `*:11434` или `0.0.0.0:11434` (НЕ 127.0.0.1:11434)
...
### Проблема: VM не запускается - Cannot allocate memory

**Симптомы:** `QEMU exited with code 1`, в логах: `Cannot allocate memory (12)`
...
**Причина:** VM пытается выделить больше RAM чем доступно на хосте.

**Решение:**

1. **Уменьшите RAM VM:**
   ```bash
   # На хосте (VM должна быть выключена)
   qm set <VMID> --memory 6144  # 6GB вместо 8GB
   ```

2. **Или через Web UI:**
   - VM → Hardware → Memory → Edit
   - Уменьшите до 4096-6144 MB
...
**Рекомендации:**
- **8GB хост:** VM максимум 6GB
- **16GB хост:** VM максимум 12GB
- **32GB хост:** VM максимум 24GB

**Долгосрочное решение:** Увеличьте RAM на Proxmox хосте до 16GB+.

---

### Проблема: AMD-Vi не инициализируется (IOMMU groups = 0)

**Симптомы:** После настройки GRUB `ls /sys/kernel/iommu_groups/` пустой, AMD-Vi не появляется в dmesg

**Диагностика:**
```bash
# 1. Проверка IVRS таблицы
dmesg | grep -i ivrs
...
1. **Вернитесь в BIOS:**
   - Убедитесь что **IOMMU = Enabled** (AMD CBS раздел)
   - Для ASRock: может потребоваться обновление BIOS до последней версии

2. **Если IVRS присутствует, но AMD-Vi не инициализируется:**
   ```bash
   # Попробуйте явно указать amd_iommu=on
   nano /etc/default/grub
   GRUB_CMDLINE_LINUX_DEFAULT="quiet amd_iommu=on iommu=pt"
   update-grub
   reboot
   ```
   (Предупреждение "Unknown option" можно игнорировать если AMD-Vi инициализируется)
...
### Проблема: Ollama недоступен по сети (слушает на 127.0.0.1)

**Симптомы:**
- В VM: `curl http://localhost:11434/api/tags` работает
- С хоста: `curl http://<VM_IP>:11434/api/tags` → Connection refused
- `sudo ss -tlnp | grep 11434` показывает `127.0.0.1:11434` вместо `0.0.0.0:11434`

**Причина:** Переменная окружения `OLLAMA_HOST` не применилась.

**Решение:**

1. **Проверьте что override.conf создан:**
   ```bash
   sudo cat /etc/systemd/system/ollama.service.d/override.conf
   ```

   Если файла нет - создайте вручную (см. Этап 10.2, Вариант B)

2. **Если файл существует, но не применился:**
   ```bash
   sudo systemctl daemon-reload
   sudo systemctl restart ollama.service
   ```
...
4. **Проверьте что теперь слушает на всех интерфейсах:**
   ```bash
   sudo ss -tlnp | grep 11434
   ```
...
5. **Если всё еще не работает - временный workaround:**
   ```bash
   sudo systemctl stop ollama.service
   OLLAMA_HOST=0.0.0.0:11434 ollama serve
   ```

   Если заработало - проблема в systemd override, пересоздайте файл.
...
**Proxmox хост:**
- ✅ IOMMU enabled в BIOS (для ASRock: Advanced → AMD CBS → IOMMU)
- ✅ BIOS передает IVRS таблицу (`dmesg | grep -i ivrs` не пусто)
- ✅ GRUB обновлен с `iommu=pt` (для AMD: `amd_iommu=on` опционально)
- ✅ IOMMU группы созданы (`ls /sys/kernel/iommu_groups/ | wc -l` > 0)
- ✅ AMD-Vi инициализирован (`dmesg | grep -i amd-vi` показывает "Interrupt remapping enabled")
- ✅ VFIO модули загружены (`lsmod | grep vfio`)
- ✅ GPU captured by vfio-pci (`lspci -nnk` показывает "Kernel driver in use: vfio-pci")
...
## Известные проблемы и решения (из реального опыта)

### 1. ⚠️ ASRock материнские платы: IOMMU скрыт в старых BIOS
**Проблема:** В BIOS версии P3.xx опция IOMMU может отсутствовать
**Решение:** Обновление BIOS до P7.xx+ (см. Этап 1.3)

### 2. ⚠️ AMD процессоры: параметр amd_iommu=1 не работает
**Проблема:** В ядре 6.8+ параметр `amd_iommu=1` вызывает "Unknown option"
**Решение:** Используйте `amd_iommu=on` (предупреждение можно игнорировать)

### 3. ⚠️ Недостаток RAM на хосте
**Проблема:** VM не запускается с ошибкой "Cannot allocate memory"
**Решение:** Уменьшите RAM VM (см. Этап 4.5 и Troubleshooting)

### 4. ⚠️ Дублирование параметра cpu в конфиге VM
**Проблема:** После ручного редактирования остается старая строка `cpu: host`
**Решение:** Удалите старую строку, оставьте только с параметрами (см. Этап 6.2)

### 5. ⚠️ Майнинговые карты: x-vga=1 вызывает проблемы
**Проблема:** P106/P104 без видеовыходов не работают с x-vga=1
**Решение:** Не используйте x-vga=1 для майнинговых карт (см. Этап 6.1)

### 6. ⚠️ Ollama слушает только на 127.0.0.1
**Проблема:** override.conf не создается автоматически
**Решение:** Создайте файл вручную и проверьте (см. Этап 10.2, Вариант B)

### 7. ✅ EFI раздел не смонтирован (не критично)
**Проблема:** Предупреждения при update-initramfs
**Решение:** Опционально - смонтировать /dev/sda2 в /boot/efi

### 8. ✅ Предупреждение "Unknown option - 'on'" для amd_iommu
**Не проблема:** AMD-Vi инициализируется корректно несмотря на предупреждение
**Действие:** Игнорируйте, проверяйте по IOMMU groups

---

**Автор:** AI Assistant (Proxmox DevOps Expert)
**Консолидация:** Community Best Practices + Deep Proxmox Knowledge + Real Deployment Experience
