...
# Индикация (внешний LED на GPIO5)
blue_led_pin: GPIO5  # Горит когда радар детектирует цели
                     # GPIO2 нельзя - это strapping pin!
...
- 🔵 **Горит** - радар получает валидные данные (есть цели)
- ⚫ **Не горит** - радар не видит целей
- **Пин:** GPIO5 (подключите внешний LED)
- ⚠️ **GPIO2 НЕЛЬЗЯ** использовать - это strapping pin (вызывает bootloop!)
- Можно изменить пин в `substitutions: blue_led_pin` (рекомендуется GPIO5/18/19/21/22/23)
...
│  RX       ──┼───────────┼─── GPIO17 TX │
│             │           │              │
│             │           │ GPIO5 ───────┼─── 🔵 LED (опционально)
│             │           │              │    + резистор 220Ω → GND
└─────────────┘           └──────────────┘

⚠️ GPIO2 НЕЛЬЗЯ использовать - это strapping pin (вызывает bootloop)!
   Используйте GPIO5/18/19/21/22/23 для внешнего LED.
```
...
- 🔵 **Светодиод (GPIO5)** - опциональный внешний LED
  - Горит когда радар получает валидные данные
  - Позволяет визуально контролировать работу без Home Assistant
  - Подключение: GPIO5 → LED → резистор 220Ω → GND
  - ⚠️ **Не используйте GPIO2** - это strapping pin (вызывает bootloop!)
...
**Проверка работы:**
- Если подключили внешний LED на GPIO5 - он должен загореться при детектировании
- Подвигайтесь перед датчиком - LED должен реагировать
- Проверьте логи на наличие данных от радара
- Если нет данных - проверьте подключение TX/RX проводов
